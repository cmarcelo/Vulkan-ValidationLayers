...
void CoreChecks::SetDisabledFlags(layer_data *device_data, const VkValidationFlagsEXT *val_flags_struct) {
    for (uint32_t i = 0; i < val_flags_struct->disabledValidationCheckCount; ++i) {
        switch (val_flags_struct->pDisabledValidationChecks[i]) {
            case VK_VALIDATION_CHECK_SHADERS_EXT:
                device_data->disabled.shader_validation = true;
                break;
            case VK_VALIDATION_CHECK_ALL_EXT:
                // Set all disabled flags to true
                device_data->disabled.SetAll(true);
                break;
            default:
                break;
        }
...
void CoreChecks::SetValidationFeatures(layer_data *device_data, const VkValidationFeaturesEXT *val_features_struct) {
    ...
    for (uint32_t i = 0; i < val_features_struct->enabledValidationFeatureCount; ++i) {
        switch (val_features_struct->pEnabledValidationFeatures[i]) {
            case VK_VALIDATION_FEATURE_ENABLE_GPU_ASSISTED_EXT:
                enabled.gpu_validation = true;
                break;
            case VK_VALIDATION_FEATURE_ENABLE_GPU_ASSISTED_RESERVE_BINDING_SLOT_EXT:
                enabled.gpu_validation_reserve_binding_slot = true;
                break;
            default:
                break;
        }
...
bool CoreChecks::PreCallValidateBeginCommandBuffer(const VkCommandBuffer commandBuffer,
...
void CoreChecks::PostCallRecordGetPhysicalDeviceProperties(VkPhysicalDevice physicalDevice,
